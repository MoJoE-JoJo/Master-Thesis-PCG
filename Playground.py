...
#--------------------------------------------------------------------------------------------
arl_save_folder = "saved_arl/16/"
generated_level_path = os.path.dirname(os.path.realpath(__file__)).replace("\\MAFGym", "") + "\\generated_levels\\"
arl = ARLPCG(load_path="", levels_path=level_folder, generate_path=generated_level_path, save_name="arl-dev", internal=10, external=0.8, pcg_env_type=PCGEnvType.SIM)
...
while run:
    ...
    if(new_time - start_time >= total_runtime):
        ...
        arl.train(False)
validate_arl(arl, 100, 10, "16_arl")
