...
def validate_agent(env, agent_path, num_of_val_plays, saveName):
    allCheckpoints = onlyfiles = [f for f in listdir(agent_path) if isfile(join(agent_path, f))]
...
validate_agent(env, "saved_agents/old_new_env/old_env/lvl_1/", 100, "old_env;5e-5;lvl_1")
print("1 done")

levelFilePath = os.path.dirname(os.path.realpath(__file__)) + "\\MAFGym\\levels\\original\\lvl-2.txt"
levelString = readLevelFile(levelFilePath)
env.setLevel(levelString)
validate_agent(env, "saved_agents/old_new_env/old_env/lvl_2/", 100, "old_env;5e-5;lvl_2")
print("2 done")

levelFilePath = os.path.dirname(os.path.realpath(__file__)) + "\\MAFGym\\levels\\original\\lvl-7.txt"
levelString = readLevelFile(levelFilePath)
env.setLevel(levelString)
validate_agent(env, "saved_agents/old_new_env/old_env/lvl_7/", 100, "old_env;5e-5;lvl_7")
print("7 done")

levelFilePath = os.path.dirname(os.path.realpath(__file__)) + "\\MAFGym\\levels\\original\\lvl-8.txt"
levelString = readLevelFile(levelFilePath)
env.setLevel(levelString)
validate_agent(env, "saved_agents/old_new_env/old_env/lvl_8/", 100, "old_env;5e-5;lvl_8")
print("8 done")